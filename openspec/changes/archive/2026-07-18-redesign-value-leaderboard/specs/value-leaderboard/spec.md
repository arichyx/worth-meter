...
**Reason**: Target-dependent value recovered excludes most active assets and incorrectly implies that unlike asset types share one rankable unit.

**Migration**: `/leaderboard` remains available and now provides a target-independent overview plus type-specific detailed rankings.

### Requirement: Value score SHALL be comparable across asset types
**Reason**: Cost per day, cost per use, and utilization percentage are not directly comparable without arbitrary normalization.

**Migration**: Compare assets within their type using the metric displayed beside each rank.

### Requirement: Assets without a measurable target SHALL sort to the bottom
**Reason**: Optional targets must not gate core leaderboard usefulness.

**Migration**: Assets without targets rank normally from observed data; only zero-use count assets wait outside the numbered ranking.

### Requirement: The leaderboard SHALL surface a champion and a biggest regret
**Reason**: Champion and regret selection depended on the removed cross-type target score.

**Migration**: The overview shows a best-value asset per type and concrete, non-judgmental attention insights.
